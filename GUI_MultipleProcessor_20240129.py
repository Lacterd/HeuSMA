...
class HeuristicProcessorUI(QWidget):
    ...
    def Run(self):
        ...
        '''
        def assine_MS2(RefList,MergeMS2,temp_MZ_Tor,Row_index):
            Valified_List = list(filter(lambda x:abs(RefList.at[i,'m/z']-MergeMS2.at[x,'Pre_MZ'])/RefList.at[i,'m/z']<temp_MZ_Tor and RefList.at[i,'RI']-6<MergeMS2.at[x,'Scan_RI']<RefList.at[i,'RI']+6,range(len(MergeMS2))))
            if len(Valified_List) >0:
                Valified_Index = np.where(abs(np.array(MergeMS2.loc[Valified_List,'Scan_RI'])-RefList.at[i,'RI'])==min(abs(np.array(MergeMS2.loc[Valified_List,'Scan_RI'])-RefList.at[i,'RI'])))[0][0]
                temp_MZ_List = list(MergeMS2.at[Valified_List[Valified_Index],'MZ_List'])
                temp_Int_List = list(MergeMS2.at[Valified_List[Valified_Index],'Int_List'])
                return [temp_MZ_List,temp_Int_List]
            else:
                return [[0],[0]]
        self.pool_result = {}
        pool = mp.Pool(os.cpu_count()-2)
        RefList = self.Align.RefList.copy()
        MergeMS2 = self.MS2_Data.copy()
        for i in range(len(self.Align.RefList)):
            self.pool_result[i] = pool.apply_async(assine_MS2,(RefList,MergeMS2,temp_MZ_Tor,i))
        pool.close()
        pool.join()
        for i in pool_result.keys():
            self.Align.RefList.at[i,'MS2_MZ'] = self.pool_result[i].get()[0].copy()
            self.Align.RefList.at[i,'MS2_Int'] = self.pool_result[i].get()[1].copy()
        '''
        ''' Filter '''
        for i in ClassList:
