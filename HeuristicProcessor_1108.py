...
class HeuristicProcessorUI(QWidget):
    ...
    def initUI(self):
        ...
        # Union
        self.Label_SampleSelect = QLabel('Select datas')
        #self.Label_BlankSelect = QLabel('Select blank datas')
        self.Label_RIISSelect = QLabel('Select RIIS datas')
        ...
        #self.TextBrowser_BlankSelect = QTextBrowser()
        ...
        #self.PushButton_BlankSelect = QPushButton('Select')
        ...
        #self.PushButton_BlankSelect.setToolTip('Select blank *.mzML documents')
        ...
        #self.PushButton_BlankSelect.clicked.connect(self.SelectBlankFile)
        ...
        #Data_import_Layout.addWidget(self.Label_BlankSelect,3,0)
        #Data_import_Layout.addWidget(self.TextBrowser_BlankSelect, 4, 0)
        #Data_import_Layout.addWidget(self.PushButton_BlankSelect, 4, 1)
        ...
        self.LineEdit_RT_Tor = QLineEdit('5')
        ...
        Params_setting_Layout.addWidget(self.Lable_MS_Tor,0,0)
        Params_setting_Layout.addWidget(self.LineEdit_MS_Tor,0,1)
        Params_setting_Layout.addWidget(self.Lable_RT_Tor,0,2)
        Params_setting_Layout.addWidget(self.LineEdit_RT_Tor,0,3)
        Params_setting_Layout.addWidget(self.Lable_Int_min,0,4)
        Params_setting_Layout.addWidget(self.LineEdit_Int_min,0,5)
        Params_setting_Layout.addWidget(self.Lable_Point,1,0)
        Params_setting_Layout.addWidget(self.LineEdit_Point,1,1)
        Params_setting_Layout.addWidget(self.Lable_SN,1,2)
        Params_setting_Layout.addWidget(self.LineEdit_SN,1,3)
        Params_setting_Layout.addWidget(self.Lable_SB,1,4)
        Params_setting_Layout.addWidget(self.LineEdit_SB,1,5)
        ...
        '''
        self.TextBrowser_ProcessBar = QTableWidget()
        self.TextBrowser_ProcessBar.setColumnCount(2)
        self.TextBrowser_ProcessBar.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.ResizeToContents)
        self.TextBrowser_ProcessBar.setHorizontalHeaderLabels(['Name','Process percentage'])
        self.TextBrowser_ProcessBar.horizontalHeaderItem(0).setTextAlignment(Qt.AlignCenter)
        self.TextBrowser_ProcessBar.horizontalHeaderItem(1).setTextAlignment(Qt.AlignCenter)
        self.TextBrowser_ProcessBar.horizontalHeaderItem(0).setSizeHint(QtCore.QSize(int(self.TextBrowser_ProcessBar.viewport().width()/5),20))
        self.TextBrowser_ProcessBar.horizontalHeaderItem(1).setSizeHint(QtCore.QSize(int(self.TextBrowser_ProcessBar.viewport().width()*4/5),20))
        StatusBar_Layout.addWidget(self.TextBrowser_ProcessBar,0,0)
        '''
    ...
    def SelectSampleFile(self):
        FileName,FileType = QFileDialog.getOpenFileNames(self,"选取文件",os.getcwd(),"All Files(*)")
        for i in FileName:
            Combox_Type = QComboBox()
            Combox_Type.addItems(['Sample','QC','Blank'])
    ...
    def SelectRIISFile(self):
        FileName,FileType = QFileDialog.getOpenFileName(self,"选取文件",os.getcwd(),"All Files(*)")
        name_begin = FileName.rfind('/')
        if name_begin == -1:
            name_begin = FileName.rfind('\\')
    ...
    def Run(self):
        ...
        self.SampleClass = {}
        for i in self.Data_params.keys():
            if self.Data_params[i]['Type'] != 'Blank':
        ...
        name_begin = self.SampleData[list(main.SampleData.keys())[0]]['Path'].rfind('/')
        if name_begin == -1:
            name_begin = self.SampleData[list(main.SampleData.keys())[0]]['Path'].rfind('\\')
        self.Align_path = self.SampleData[list(main.SampleData.keys())[0]]['Path'][0:name_begin+1]+'Alignment-'+str(time.localtime()[1])+str(time.localtime()[2])+str(time.localtime()[3])+str(time.localtime()[4])+'.xlsx'
        for i in self.SampleData.keys():
            self.SampleData[i]['Data'] = EazyMZDataProcess(self.SampleData[i]['Path'])
            self.SampleData[i]['Data'].set_param('MS1_Tor',float(self.LineEdit_MS_Tor.text())/1000000)
            self.SampleData[i]['Data'].set_param('RT_Tor',int(self.LineEdit_Point.text()))
        ...
        for i in self.BlankData.keys():
            ...
            self.BlankData[i]['Data'].set_param('RT_Tor',int(self.LineEdit_Point.text()))
        self.HeuristicList = []
        self.Label_process_sub.setText('Set Heuristic List')
        for i in self.SampleData.keys():
            if len(self.HeuristicList) == 0:
                self.SampleData[i]['Data'].add_Heuristic_peak_list(self.HeuristicListPath)
                self.HeuristicList = self.SampleData[i]['Data'].Heuristic_peak_list.copy()
            else:
        ...
        for i in self.SampleData.keys():
            PN += 1
            self.process_bar.setValue(PN)
            QApplication.processEvents()
            pool_result[i]=pool.apply_async(self.SampleData[i]['Data'].Heuristic_PeakDetect)
        pool.close()
        pool.join()
        for i in self.SampleData.keys():
        ...
        for i in self.SampleData.keys():
            ...
            self.SampleData[i]['Data'].Final_Peak_Detect.to_excel(self.SampleData[i]['Path'][0:len(self.SampleData[i]['Path'])-5]+'-'+str(time.localtime()[1])+str(time.localtime()[2])+str(time.localtime()[3])+str(time.localtime()[4])+'.xlsx',index=False)
        self.Label_process.setText('Data Alignment')
        self.Align = DataAlignment()
        for i in self.SampleData.keys():
        ...
        self.Align.set_param('RT_Tor',float(self.LineEdit_RT_Tor.text())/100)
        ...
        for ii in range(len(self.Align.RefList)):
            WheterDel = True
            for i in ClassList:
                if self.Align.RefList['Group '+str(i)][ii] > 0.8:
                    WheterDel = False
            if WheterDel == True:
                Repeat_Filter.append(ii)
        self.Align.RefList.drop(Repeat_Filter,inplace=True)
        '''
...
class EazyMZDataProcess(object):   
    ...
    def add_Heuristic_peak_list(self,path):
        ...
        for i in list(set(Heuristic_peak_list['Gradient Code'])):
            ...
            sub_Heuristic_peak_list = sub_Heuristic_peak_list.copy()
            temp_Heuristic_peak_list = []
            for ii in range(len(sub_Heuristic_peak_list)):
                if len(temp_Heuristic_peak_list) == 0:
                    temp_Heuristic_peak_list = sub_Heuristic_peak_list.iloc[[0],:].copy()
                    temp_Heuristic_peak_list['PeakNumber'] = 1
                else:
                    MZ = sub_Heuristic_peak_list.at[ii,'AverageMZ']
                    RI_left = sub_Heuristic_peak_list.at[ii,'RI_left']
                    RI_right = sub_Heuristic_peak_list.at[ii,'RI_right']
                    RI = sub_Heuristic_peak_list.at[ii,'RI']
                    same_list = list(filter(lambda x:EazyMZDataProcess.ppm_compare(MZ,temp_Heuristic_peak_list.at[x,'AverageMZ'])<0.000010 and 
                    ...
                    contain_list = list(filter(lambda x:EazyMZDataProcess.ppm_compare(MZ,temp_Heuristic_peak_list.at[x,'AverageMZ'])<0.000010 and (RI_left>=temp_Heuristic_peak_list.at[x,'RI_left'] and RI_right<=temp_Heuristic_peak_list.at[x,'RI_right']),range(len(temp_Heuristic_peak_list))))
                    if len(same_list) == 0 :
                        if len(contain_list) == 0:
                            iii = sub_Heuristic_peak_list.iloc[[ii],:].copy()
                            iii['PeakNumber'] = 1
                        ...
                        iii.at[ii,'RI_List'] = list(temp_Heuristic_peak_list.loc[same_list[0],'RI_List'] + temp_Heuristic_peak_list.loc[same_list[1],'RI_List'] + iii.loc[ii,'RI_List'])
                        iii.loc[ii,'AverageMZ'] = float(np.mean(iii.loc[ii,'MZ_List']))
                        temp_Heuristic_peak_list.drop(same_list,inplace=True)
                        temp_Heuristic_peak_list = pd.concat([temp_Heuristic_peak_list,iii])
                        temp_Heuristic_peak_list.reset_index(drop=True,inplace=True)
            if len(Alltemp_Heuristic_peak_list) == 0:
                Alltemp_Heuristic_peak_list = temp_Heuristic_peak_list.copy()
            else:
                Alltemp_Heuristic_peak_list = pd.concat([Alltemp_Heuristic_peak_list,temp_Heuristic_peak_list])
                Alltemp_Heuristic_peak_list.reset_index(drop=True,inplace=True)
        self.Heuristic_peak_list = Alltemp_Heuristic_peak_list.copy()
        '''
        ...
        for i in sub_HPL_dict.keys():
            ...
            for ii in range(len(Alltemp_HPL)):
                if len(init_RefMZ)==0:
                    ...
                    if abs(init_RefMZ.at[len(init_RefMZ)-1,'AverageMZ']-Alltemp_HPL.at[ii,'AverageMZ'])/Alltemp_HPL.at[ii,'AverageMZ']<0.000010:
            ...
            for ii in range(len(temp_Data)):
                if len(init_SampleMZ)==0:
                    ...
                    if abs(init_SampleMZ.at[len(init_SampleMZ)-1,'AverageMZ']-temp_Data.at[ii,'AverageMZ'])/temp_Data.at[ii,'AverageMZ']<0.000010:
            ...
            for ii in range(len(init_SampleMZ)):
                ...
                match_Index = list(filter(lambda x:abs(init_RefMZ.at[x,'AverageMZ']-Sample_MZ)/Sample_MZ<0.000010,range(len(init_RefMZ))))
                ...
                for i_Ref in range(len(Ref_Index)):
                    for i_Sample in range(len(init_SampleMZ.at[ii,'RowIndex'])):
                        ...
                        if abs(i_Ref_MZ-i_Sample_MZ)/i_Ref_MZ<0.000010 and abs(i_Ref_Time-i_Sample_Time)/i_Ref_Time<0.02:
                            Score_matrix[i_Ref,i_Sample] = 0.5*np.exp(-0.5*((i_Sample_Time-i_Ref_Time)/(i_Ref_Time*0.02))**2)+(1-0.5)*np.exp(-0.5*((i_Sample_MZ-i_Ref_MZ)/(i_Sample_MZ*0.000010))**2)
        ...
        Alltemp_HPL['RI_left'] = Alltemp_HPL['RI'].apply(lambda x:x*0.98)
        Alltemp_HPL['RI_right'] = Alltemp_HPL['RI'].apply(lambda x:x*1.02)
    ...
    def Heuristic_PeakDetect(self):
        ...
        Final_Peak_Detect = pd.DataFrame(columns=['AverageMZ', 'RT', 'RI', 'Int','RTList'])
        ...
        for ii in range(len(Heuristic_peak_list)):   
            ...
            if len(Begin_Place) >= 1 and len(End_Place) >= 1 and Begin_Place[0] < End_Place[-1]:
                ...
                temp_Final_Peak = pd.DataFrame(columns=['AverageMZ', 'RT', 'RI', 'Int','RTList'])
                for v in range(len(Peak_Begin)):
                    ...
                    #rate = max(Int_List[Peak_Begin[v]],Int_List[Peak_End[v]])/Int
                    if Int > self.__param['min_Int'] and Int/Noise>=3 and len(RTList)>=self.__param['Points'] and len(list(filter(lambda x:0.98<RI/x<1.02,RI_List)))>=1 : 
                        #IntList = Int_List[Peak_Begin[v]:Peak_End[v]+1]
                        Same_list = list(filter(lambda x:abs(Final_Peak_Detect['AverageMZ'][x]-MZ)/MZ<0.000010 and abs(Final_Peak_Detect['RT'][x]-RT)<2,range(len(Final_Peak_Detect))))
                        if len(Same_list) == 0:
                            temp_Final_Peak_v = pd.DataFrame([(MZ, RT, RI, Int,RTList)], columns=['AverageMZ', 'RT', 'RI', 'Int','RTList'])
        ...
        #self.Heuristic_peak_list = Heuristic_peak_list.copy()
    ...
    def set_RIIS(self,RIIS):
        ...
        if 'RT' not in list(self.RIIS.keys()):
            if hasattr(self,'Final_Peak_Detect') == False:
                self.RIIS['RT']=0.00
                RTL = 60
                ...
                for ii in range(len(self.RIIS)):                
                    ...
                    self.RIIS['RT'][ii] = RT_List[RT_place[0]]
            else:
...
class DataAlignment(object):
    def __init__(self):
        ...
        self.AlignmentParam = {'MZ_Tor':0.000010,'RT_Tor':0.02,'A':0.5,
        ...
        for i in range(len(self.DataBase)):
            if self.DataBase.at[i,'Tag'] != 'Blank':
                ...
                add_RefList = pd.DataFrame({'m/z': add_MZ,'RT': add_RT,self.DataBase.at[i, 'Data_Name']: add_Int,
                                            'MS_List':add_MS_List,'RT_List':add_RT_List})
