...
class HeuristicProcessorUI(QWidget):
    ...
    def Run(self):
        ...
        for i in range(len(self.Align.RefList)):
            Valified_List = list(filter(lambda x:abs(self.Align.RefList.at[i,'m/z']-self.MergeMS2.at[x,'Pre_MZ'])/self.Align.RefList.at[i,'m/z']<temp_MZ_Tor and self.Align.RefList.at[i,'RI']-6<self.MergeMS2.at[x,'Scan_RI']<self.Align.RefList.at[i,'RI']+6,range(len(self.MergeMS2))))
            if len(Valified_List) >0:
                ...
                #self.Align.RefList.loc['MS2_MZ'][i] = self.MergeMS2.at[Valified_Index,'MZ_List'].copy()
                #self.Align.RefList.loc['MS2_Int'][i] = self.MergeMS2.at[Valified_Index,'Int_List'].copy()
