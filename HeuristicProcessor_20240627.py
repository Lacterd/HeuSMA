...
class HeuristicProcessorUI(QWidget):
    ...
    def Run(self):
        ...
        if len(mgf_output)>0:
            with open(self.filepath_title+'Alignment-tandemMS-'+str(time.localtime()[1])+str(time.localtime()[2])+str(time.localtime()[3])+str(time.localtime()[4])+'.mgf','w')as mgfFile:
                mgfFile.write(mgf_output)
            csv_mgf.to_csv(self.filepath_title+'Alignment-tandemMS-'+str(time.localtime()[1])+str(time.localtime()[2])+str(time.localtime()[3])+str(time.localtime()[4])+'.csv',index=False)
...
class EazyMZDataProcess(object):   
    ...
    def OutputSinglePeakList(self):
        ...
        Data.sort_values(by='AverageMZ',ascending=True,inplace=True)
        Data.reset_index(drop=True,inplace=True)
        Data.insert(0,'ID',[0]*len(Data))
        for i in range(len(Data)):
            Data['ID'][i] = i+1
            Data['AverageMZ'][i] = np.around(Data['AverageMZ'][i],5)
            Data['RT'][i] = np.around(Data['RT'][i],2)
            Data['RI'][i] = np.around(Data['RI'][i],1)
...
class DataAlignment(object):
    ...
    def RenewRefList(self):
        ...
        if self.AlignmentParam['RI_Alignment'] == True:
            self.RefList.insert(2,'RI',[0]*len(self.RefList))
