    ...
    def Run(self):
        ...
        for i in self.Data_params.keys():
            if self.Data_params[i]['Type'] == 'Sample':
                ...
                self.MS_2Data[i] = self.Data_params[i]
            else:
        ...
        self.Align.set_param('MZ_Tor',0.000010)
        self.Align.set_param('RT_Tor',0.01)
        ...
        self.MergeMS2 = []
        for i in self.MS2_Data.keys():
            self.SampleData[i]['Data'].Calculate_MS2_RI()
            if len(self.MergeMS2) == 0:
                self.MergeMS2 = self.SampleData[i]['Data'].MS2_Data.copy()
            else:
                self.MergeMS2 = pd.concat([self.MergeMS2,self.SampleData[i]['Data'].MS2_Data])
        if len(self.MergeMS2)>0:
        ...
        for i in range(len(self.Align.RefList)):
            Valified_List = list(filter(lambda x:EazyMZDataProcess.ppm_compare(self.Align.RefList.at[i,'m/z'],self.MergeMS2.at[x,'Pre_MZ']<self.Align.AlignmentParam['MZ_Tor']) and self.MergeMS2.at[x,'Scan_RI']-6<self.Align.RefList.at[i,'RI']<self.MergeMS2.at[x,'Scan_RI']+6,range(len(self.MergeMS2))))
            if len(Valified_List) >0:
                Valified_Index = list(filter(lambda x:self.MergeMS2.at[x,'Rel_Int']==max(self.MergeMS2['Rel_Int'][Valified_List]),Valified_List))[0]
                self.Align.RefList.iloc[i,'MS2_MZ'] = self.MergeMS2.at[Valified_Index,'MZ_List'].copy()
                self.Align.RefList.iloc[i,'MS2_Int'] = self.MergeMS2.at[Valified_Index,'Int_List'].copy()
        ''' Filter '''
        for i in ClassList:
        ...
        self.Align.RefList.to_excel(self.SampleData[list(main.SampleData.keys())[0]]['Path'][0:name_begin+1]+'Alignment-Origin-'+str(time.localtime()[1])+str(time.localtime()[2])+str(time.localtime()[3])+str(time.localtime()[4])+'.xlsx',index=False)
        self.Align.BlankFilter()
        self.Align.RefList.to_excel(self.SampleData[list(main.SampleData.keys())[0]]['Path'][0:name_begin+1]+'Alignment-BlankRemove-'+str(time.localtime()[1])+str(time.localtime()[2])+str(time.localtime()[3])+str(time.localtime()[4])+'.xlsx',index=False)
        '''
        Repeat_Filter = []
        for ii in range(len(self.Align.RefList)):
            WheterDel = True
        ...
        with open(self.SampleData[list(main.SampleData.keys())[0]]['Path'][0:name_begin+1]+'Alignment-MS2-MGF-'+str(time.localtime()[1])+str(time.localtime()[2])+str(time.localtime()[3])+str(time.localtime()[4])+'.mgf','w')as mgfFile:
            mgfFile.write(mgf_output)
        csv_mgf.to_csv(self.SampleData[list(main.SampleData.keys())[0]]['Path'][0:name_begin+1]+'Alignment-MS2-CSV-'+str(time.localtime()[1])+str(time.localtime()[2])+str(time.localtime()[3])+str(time.localtime()[4])+'.csv',index=False)
        self.process_bar.setMaximum(100)
    ...
    def Heuristic_PeakDetect(self):
        ...
        for ii in range(len(Heuristic_peak_list)):   
            ...
            if Heuristic_peak_list.at[ii,'PeakNumber'] == 1:
                ...
                FD_Median_N = FD_Median*(-1)
                SD_Median = sum(SD_List[(min(SD_List)*0.1<=SD_List) & (SD_List<=0)])/len(SD_List[(min(SD_List)*0.1<=SD_List) & (SD_List<=0)])
            ...
            if len(Begin_Place) >= 1 and len(End_Place) >= 1 and Begin_Place[0] < End_Place[-1]:
                ...
                for i_wave in range(len(PeakDetect_Table)):
                    if PeakDetect_Table.at[i_wave,'Begin_Place'] >= 5:
                        Begin_min = list(filter(lambda x:min(Int_List[PeakDetect_Table.at[i_wave,'Begin_Place']-5:PeakDetect_Table.at[i_wave,'Begin_Place']])==Int_List[x],range(PeakDetect_Table.at[i_wave,'Begin_Place']-5,PeakDetect_Table.at[i_wave,'Begin_Place'])))
                        PeakDetect_Table.at[i_wave,'Begin_Place'] = Begin_min[0]
                    if PeakDetect_Table.at[i_wave,'End_Place'] < len(Int_List)-5:
                        End_min = list(filter(lambda x:min(Int_List[PeakDetect_Table.at[i_wave,'End_Place']:PeakDetect_Table.at[i_wave,'End_Place']+5])==Int_List[x],range(PeakDetect_Table.at[i_wave,'End_Place'],PeakDetect_Table.at[i_wave,'End_Place']+5)))
                ...
                for ii in range(len(init_SampleMZ)):
                    ...
                    for i_Ref in range(len(Ref_Index)):
                        for i_Sample in range(len(init_SampleMZ.at[ii,'RowIndex'])):
                            ...
                            i_Sample_Time = Sample_Time[init_SampleMZ.at[ii,'RowIndex'][i_Sample]]
                            if abs(i_Ref_MZ-i_Sample_MZ)/i_Ref_MZ<self.AlignmentParam['MZ_Tor'] and abs(i_Ref_Time-i_Sample_Time)<6:
